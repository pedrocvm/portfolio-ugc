...
test('fila vazia com oportunidades explica o silêncio em vez de mostrar um zero', () => {
  ...
  assert.match(s, /Nada à espera de ti hoje/);
...
test('uma só oportunidade não é «1 conversas»', () => {
  ...
  assert.match(s, /a conversa que tens em aberto/);
...
test('com fila, diz quantas e por onde começar', () => {
  ...
  assert.match(s, /Tens 4 coisas para fazer hoje, 2 já fora de prazo\./);
  assert.match(s, /Começa pela Cecotec, que passou do prazo há 3 dias\./);
});
...
test('uma coisa no singular, e um dia no singular', () => {
  ...
  assert.match(s, /Tens 1 coisa para fazer hoje/);
...
test('sem atraso, a marca é nomeada sem inventar uma espera', () => {
  ...
  assert.match(s, /Começa pela Xiaomi\./);
